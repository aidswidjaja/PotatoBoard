...
def index(request):
    ...
    emergency = Emergency.objects.order_by('-date_created')[:1]   # this will find the most recent iteration of the emergency msg
...
def all(request):
    post_list_all = Post.objects.order_by('-date_created')
    emergency = Emergency.objects.order_by('-date_created')[:1]
